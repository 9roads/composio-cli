...
import { detectCliPlatform, supportsCliPlatform } from './platform.js';
import type {
...
export const getLocalToolsBundleRoot = (): string => {
  ...
  return candidates.find(candidate => fsSync.existsSync(candidate)) ?? candidates[0] ?? DEFAULT_BUNDLE_DIR;
};
