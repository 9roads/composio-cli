...
import { beeperImessageToolkit } from './toolkits/beeper-imessage.js';
import { chromeDevtoolsToolkit } from './toolkits/chrome-devtools.js';
import { peekabooToolkit } from './toolkits/peekaboo.js';
import { formatSupportedPlatforms, detectCliPlatform, supportsCliPlatform } from './platform.js';
import { executeLocalTool } from './runtime.js';
import type {
  LocalCliPlatform,
...
const toJsonSchema = (schema: LocalToolDeclaration['inputParams']): Record<string, unknown> =>
...
const toCustomTool = (
  ...
  inputSchema: toJsonSchema(tool.inputParams),
  ...(tool.outputParams ? { outputSchema: toJsonSchema(tool.outputParams) } : {}),
  execute: async input =>
